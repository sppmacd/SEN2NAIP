...
def load_image(roi: str) -> tuple[np.ndarray, np.ndarray]:
...
# Iterate over images from ROI_0000 to ROI_2850 inclusive
range_ = list(range(2851))
np.random.shuffle(range_)
for roi_num in range_:
    roi = f"ROI_{roi_num:04d}"  # Format the ROI number
    try:
        image = load_image(roi)
        show_image(roi, image)
